...
export default class QueryEditorPanel extends I18nMixin(LightningElement) {
  ...
  insertItem(event) {
    ...
    const selectedItem = this.completionItems.find(item => item.key === key);
    const inputEl = this.template.querySelector('.soql-input');
  ...
  handleBlurSoql() {
    ...
    this._closeCompletionTimer = setTimeout(() => this._closeCompletion(), 100);
  }
  ...
  _openCompletion(event) {
    ...
    this._selectionStart =
      this._findSeparator(target.value.substring(0, target.selectionEnd)) ||
      target.selectionEnd;
